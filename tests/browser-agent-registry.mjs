...
assert.equal(metadata.name, "黑盒冠军 · Triad");
...
assert.equal(agent.name, "黑盒冠军 · Triad");
assert.equal(agent.browserAgentId, TRIAD_CHAMPION_AGENT_ID);
assert.equal(agent.observationDelayFrames, 12);
...
function runFairBrowserMatch(seeds) {
...
const fairMatch = runFairBrowserMatch(oldFixedSeeds);
assert.equal(fairMatch.match.matchWinner, 1, "Triad should win the exact reported matchup behind the shared V1 boundary");
assert.deepEqual(fairMatch.match.score, [0, 2]);
...
for (const decision of fairMatch.decisions) {
  assert.equal(decision.opponentObservedFrame, decision.decisionFrame - 12);
}

const rematchSeeds = seedSequence.next();
assert.deepEqual(rematchSeeds, { matchIndex: 1, left: 20260721, right: 20260722 });
const rematch = runFairBrowserMatch(rematchSeeds);
...
for (const id of [...BUILT_IN_IDS, TRIAD_CHAMPION_AGENT_ID]) {
  ...
  assert.equal(runner.observationDelayFrames, 12, `${id} must receive platform delay once`);
}
...
process.stdout.write("browser-agent-registry ok · fair V1 roster, fresh rematch seeds, frozen hash\n");
